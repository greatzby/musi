"""
rl_grpo_2hop.py  (v3: OOM-fixed — fused cross_entropy + cleanup + empty_cache)
...
# ★★★★★ FIXED: 使用 fused cross_entropy,不物化完整 softmax,省 ~10 GiB 峰值 ★★★★★
def token_logprobs(model, input_ids, attention_mask):
    """
    Compute per-token log-prob of input_ids[:, t] under model conditioned on
    input_ids[:, :t]. Returns shape [B, T] (the first column is 0).

    Memory-efficient version using fused F.cross_entropy:
    - Avoids materializing [B, T-1, V] softmax tensor (was 5+ GiB in fp32)
    - Avoids `logits.float()` copy (another 5 GiB)
    - Internal kernel is numerically stable for bf16 input
    """
    out = model(input_ids=input_ids, attention_mask=attention_mask, use_cache=False)
    logits = out.logits[:, :-1, :]            # [B, T-1, V] in bf16
    labels = input_ids[:, 1:]                 # [B, T-1]
    B, Tm1, V = logits.shape

    # Fused log_softmax + gather, no large intermediate tensor materialized.
    nll = F.cross_entropy(
        logits.reshape(-1, V),
        labels.reshape(-1),
        reduction="none",
    ).to(torch.float32)                       # only the small [B*(T-1)] tensor goes to fp32
    log_probs = (-nll).view(B, Tm1)

    # Free the giant logits tensor ASAP.
    del out, logits, nll

    zero = torch.zeros(B, 1, device=log_probs.device, dtype=log_probs.dtype)
    return torch.cat([zero, log_probs], dim=1)
...
def compute_grpo_loss(model, ref_model, sequences, attention_mask, completion_mask,
    ...
    pg_loss_unmasked = torch.max(-adv * ratio, -adv * clipped_ratio)
    pg_loss = masked_mean(pg_loss_unmasked, completion_mask)
    ...
    # ★ 释放中间张量,降低峰值
    del log_ratio, ratio, clipped_ratio, pg_loss_unmasked
...
def main():
    ...
    while global_step < args.max_steps:
        for batch_examples in train_loader:
            ...
            # ★ 释放 generation 阶段产生的 KV cache 碎片
            ...
            # ★ 每步训练结束再清一次,防 KV-cache + autograd 残留累积碎片
            del sequences, attention_mask, completion_mask, old_logps, loss
            torch.cuda.empty_cache()
