...
ROOT_DIR="${ROOT_DIR:-checkpoints/musique-two-base-factorial-v3}"
LOG_DIR="${LOG_DIR:-logs/musique-two-base-factorial-v3}"
EVAL_OUT_DIR="${EVAL_OUT_DIR:-eval_results/musique-two-base-factorial-v3}"
...
# The two models have already been cached on the target machine.
# If the cache is incomplete, run with:
#   OFFLINE=0 bash run_two_base_musique_sft.sh
OFFLINE="${OFFLINE:-1}"
...
echo "============================================================"
echo "MuSiQue two-base SFT factorial experiment"
echo "============================================================"
...
smoke_test_one() {
    ...
    then
        ...
        echo ""
        echo "If local cache files are missing, run:"
        echo "  OFFLINE=0 bash run_two_base_musique_sft.sh"
        exit 1
    fi
...
train_one() {
    ...
    then
        ...
        echo "Training returned success but no final model exists."
    ...
    if [[ "${#MODEL_DIRS[@]}" -eq 0 ]]; then
        ...
        if CUDA_VISIBLE_DEVICES="${EVAL_GPU}" \
            python eval_musique_factorial.py \
                ...
                --resume \
                "${LOCAL_FLAGS[@]}" \
                2>&1 | tee -a "${LOG_DIR}/full_evaluation.log"
        then
